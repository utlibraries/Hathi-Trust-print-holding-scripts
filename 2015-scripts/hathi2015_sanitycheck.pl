...
foreach $file (@files) {
  ...
  while( <FILE> ) {
    ...
    if ($inputfile =~ m/utsystem_single-part/) {
        ...
        if ($fields[1] !~ m/^(uta\.|utd\.|utep\.|utmda\.|utmb\.|utpan\.|utsa\.|utsod\.|utsph\.|ut\.)/ ) {
        ...
        if ($fields[4] !~ m/0|1/ ) {print "invalid govdoc: " . $line; next; }    
        ...
        #third is either CH or WD
        #fourth is blank or BRT
        #fifth - no format rules but only one volume per line, multiple volumes with same OCLC should be on separate lines
        #sixth is 0 or 1
        ...
        if ($fields[2] !~ m/WD|CH|LM/ ) { print "invalid BCode: " . $line; next; }
        if ($fields[3] && $fields[3] ne "BRT") { print "invalid Condition:" . $lin; next; }
        if ($fields[5] !~ m/0|1/ ) {print "invalid govdoc: " . $line; next; }    
      ...
      #things to verify:
        ...
        if ($fields[3] !~ m/0|1/ ) {print "invalid govdoc: " . $line; next; }    
